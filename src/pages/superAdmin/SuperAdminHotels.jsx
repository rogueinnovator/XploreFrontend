...
import { useGetAllHotels, useSearchHotels } from "../../api/superAdmin-api.js";
...
const SuperAdminHotels = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetAllHotels({
    page: currentPage,
    dateRange: day.item,
  });

  const {
    data: searchHotelData,
  ...
  if (isError || isSearchError) {
    return (
      <div className="h-screen w-full flex justify-center items-center font-semibold text-2xl">
        Some thing went wrong getting hotels
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-auto p-4 bg-white rounded-xl shadow-lg border-b flex-col justify-start items-start gap-4 inline-flex overflow-hidden mt-4">
      ...
      {data?.hotels?.length && searchHotelData?.hotels?.length === 0 ? (
        ...
        <div className="w-full">
          <div className="w-full flex flex-col items-start">
            ...
            <div className="flex w-full px-4 py-3 rounded-xl border border-[#e4e4e4] mb-3">
              {["Hotel Name", "Contact", "Location", "Status", "Action"].map(
                (item, index) => (
                  <div
                    key={index}
            ...
            <div className="w-full flex flex-col items-start gap-2.5">
              {(searchHotelData?.hotels || data?.hotels)?.map(
                (hotel, index) => (
                  <div
                    key={index}
                    className="flex items-center w-full h-10 px-4 bg-[#fafbff] rounded-[10px]"
                  >
                    {["name", "phone", "city", "status", "action"].map(
                      (key, i) => (
                        <div key={i} className="w-1/5 text-center py-2">
                          <div className="text-[#3d425a] text-base font-medium">
                            {key === "action" ? (
                              <Link
                                to={`/super-admin/hotel-detail/${hotel._id}`}
                                className="px-2 py-1 bg-[#08a5dc] text-white text-base font-medium rounded-lg shadow-md hover:bg-[#007bb3] focus:outline-none focus:ring-2 focus:ring-[#08a5dc] focus:ring-opacity-50 transition"
                              >
                                View Details
                              </Link>
                            ) : (

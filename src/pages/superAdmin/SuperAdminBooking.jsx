...
const SuperAdminBooking = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetAllBookings({
    page: currentPage,
    dateRange: day.item,
  });
  ...
  } = useSearchBookings({ searchQuery, page: currentPage });
  ...
  return (
    <div className="w-full h-auto p-4 bg-white rounded-xl shadow-lg border-b flex-col justify-start items-start gap-4 inline-flex overflow-hidden mt-4">
      ...
      {data?.hotels?.length === 0 && searchData?.bookings?.length === 0 ? (
        ...
        <div className="w-full">
          <div className="w-full flex flex-col items-start">
            ...
            <div className="w-full flex flex-col items-start gap-2.5">
              {(searchData?.bookings || data?.bookings)?.map(
                (booking, index) => (
                  <div
                    key={index}
                    ...
                    {[
                      ...
                      <div key={i} className="w-1/5 text-center py-2">
                        <div className="text-[#3d425a] text-base font-medium">
                          {key === "checkInDate"
                            ? new Date(booking[key]).toDateString()
                            : booking[key]}
                        </div>
                      </div>
                    ))}
